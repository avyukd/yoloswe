...
2. Find the line that CREATES it — the write, the assignment, the early return, the omission.
   Search for every site that assigns the field, not only the ones that clear it: if
   clearing a field obliges clearing a sibling, then every site that SETS it carries the
   same obligation, and those writer sites usually live in a different module from the
   cleanup code you were reading.
