...
- performance. In particular: does a helper re-fetch, re-derive, or re-compute something its caller already resolved and had in hand? That waste is invisible from inside the helper and only shows up at the call site, so check the callers of each helper the diff adds.
- security.
