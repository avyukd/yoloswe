...
- **A finding always admits more than one fix, and triage picks which — not the fixer.** The reviewer proposes the fix it can see from the one site it saw; that proposal is evidence, not the decision. Name the cheapest fix that makes the finding untrue and the one the reviewer proposed, with the file count each implies, and take the smaller unless the larger is what the PR is *for*. This is where a round's real cost is set: the fixer works inside whatever remit triage hands it, so a fix chosen here without weighing its footprint is a scope decision made by omission. The asymmetry is severe — a claim deleted is one file, a claim implemented is a contract with consumers — and it recurs hardest when the finding is "X is documented/claimed but does not exist", where correcting the claim and building the thing are both valid readings of the same sentence.
...
**Widening a contract is a fix with a blast radius — find the consumers before you take it.** A fix that adds a value to an enum, a field to a payload, a state to a status, or an arm to a protocol is not local, however small its diff: every existing consumer already handles the old set exhaustively, so a new member lands in each of their else-branches. Producers and consumers also fail on different schedules — the producer's round goes green while the consumers stay wrong until something exercises them, which can be several rounds later and in a different codebase.

So when a fix widens a contract, enumerate the consumers **in the same round that widens it** — grep the value, the field, the type — and decide each explicitly:
...
**Research the class before editing.** When a finding names a mechanism, helper, contract, or an `invariant` seen in a prior round, dispatch one subagent per class (group cross-backend findings first) to enumerate every site and judge, against `$PR_SUMMARY` and the round-1 file set, which are this PR's job. Skip it for typos and single-line local fixes. It researches and recommends; **you decide and edit** — its scope call is input, the Step 3.c Scope contract still binds.

`sites_found`/`sites_fixed` record what the **agent** found, never your own count. A self-scored total measures your framing of the finding, not the class: it says "how many instances of the thing I already found", which is 1/1 by construction and stays 1/1 while the class keeps recurring. The number is only worth writing down when something other than you produced it, and `M < N` needs a reason in `notes`. An agent that always answers "1 of 1" is either right — a bounded class is a real and useful answer — or rubber-stamping; you can only tell across several rounds, so say which you think it was rather than banking the number.
